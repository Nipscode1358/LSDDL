...
class DistributedDataParallel(Module):
    ...
    def __init__(self, module, device_id=None,
                 ...
                 check_reduction=False, sparse_ratio = 0.1, sparse_threshold = 1024, mem_decay = 1.0):
    ...
    def _ddp_init_helper(self):
        ...
        self.mem = torch.zeros((self.length_all,), device = self.device_id)


        self.mask = [torch.zeros((self.sparse_length_all,),dtype = torch.long, device=self.device_id)]
        self.buckets = [None]
    ...
    def _sync_params(self):
        ...
        if self.process_group.rank() == 0:
            ...
            for p in self.module.parameters():
                if p.requires_grad:
                    temp = temp + (torch.randperm(self.flat_parameter[p].shape[0],device = self.device_id)[:self.sparse_length[p]]+self.offset_map[p],)
                    #temp = temp + (torch.randint(self.offset_map[p],self.offset_map[p]+self.flat_parameter[p].shape[0],size=(self.sparse_length[p],),device = self.device_id),)
            self.mask = [torch.cat(temp)]
    ...
    def _sync_reduction_works(self):
        ...
        self.mem = self.mem_decay * self.mem + flatten_tensor
        self.buckets = self.mem[self.mask[0]]/self.process_group.size()
        work_now = dist.all_reduce(self.buckets,async_op=True)
        self.mem[self.mask[0]] = 0
        temp_zero = torch.zeros((self.length_all,), device=self.device_id)
        work_now.wait()
